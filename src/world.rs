...
#[derive(Resource,Default,Clone)] pub struct Ui { pub can_act: bool }
#[derive(Resource,Default,Clone,Copy)] pub struct Tick { pub n:u64, pub hz:u32 }
...
pub fn setup_world(world: &mut World) {
  world.spawn((PlayerTag, Pos{ x:48, y:52 }));
  world.spawn((Monster{ id:1, hp:60, hp_max:100, kind:"SK" }, Pos{ x:51, y:54 }));
  world.spawn((Item{ id:101 }, Pos{ x:45, y:50 }));
}
